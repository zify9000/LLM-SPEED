...
Agent 场景不按上下文档位单发冷测，而是跑 **K 轮连续任务链**（上下文 append-only
增长、命中前缀缓存），固定两阶段构成：前一半短文本轮（每轮增量 ~N(1K) 正态
采样、钳制到可配区间），后一半长文本轮（默认 8K 起逐轮翻倍 ladder，起始增量
可配）；逐轮计量 TTFT、增量 prefill、decode 与缓存命中率——命中数三态诚实
呈现：服务端回传按真值；网关剥离字段（如 LiteLLM 中转）但 TTFT 相对冷轮走平
（有缓存迹象）按链内差分估算并加 ≈ 标记；无缓存迹象则标「未回传」、prefill
退回全量口径真值，不展示假设的命中数。卡片按阶段汇总——阶段一看 TTFT 均值
（高频小步循环的单步延迟），阶段二看增量 Prefill 均值（大上下文灌入的处理
能力），并附逐轮构成表（上下文长度 / 缓存命中 / 未命中 tokens）。

## ✨ 特点

| 🔌 只认协议，不认框架 | 🧮 口径透明 | 📡 实时可见 |
| --- | --- | --- |
| 挂在 OpenAI 兼容网关后的模型（llama.cpp / vLLM / LiteLLM / 云端 API）一视同仁；一个模型可映射多套部署环境、仅激活一套生效 | prefill 扣除实测网络往返、decode 滑窗差分并以 usage 权威回写、真实源码/公版文学/SWE-agent 轨迹撑上下文、随机 nonce 破 prefix cache（Agent 链除外——按真实多轮口径命中缓存并如实计量命中率）、超窗自动删减上下文重试、停滞与突发交付读数显式标记：不注水，也不被水 | prefill 等待期估值逐秒收敛、decode 逐帧刷新；跑完自动生成折线图 + 测速卡片 PNG，一键分享 |
